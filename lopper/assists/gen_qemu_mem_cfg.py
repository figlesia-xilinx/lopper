...
def is_noc_memory(compatible_list):
    """
    Check if a node is NOC memory based on compatible string.

    Filters for nodes with 'xlnx,axi-noc' compatible string.

    Args:
        compatible_list: List of compatible strings from node

    Returns:
        True if NOC memory, False otherwise
    """
    if not compatible_list:
        return False

    for compat in compatible_list:
        # Match xlnx,axi-noc-* pattern
        if re.search(r"xlnx,axi-noc", compat):
            return True
...
def extract_noc_memory_regions(tgt_node, sdt):
    ...
    for node in root_sub_nodes:
        try:
            ...
            compatible = node.propval("compatible")
            if not compatible or compatible == ['']:
                continue

            if not is_noc_memory(compatible):
                continue

            try:
                reg_value = node["reg"].value
                if not reg_value:
                    continue
            except:
                continue
